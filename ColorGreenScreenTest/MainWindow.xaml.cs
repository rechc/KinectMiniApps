...
namespace Microsoft.Samples.Kinect.ColorBasics
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void SensorColorFrameReady(object sender, ColorImageFrameReadyEventArgs e)
        {
            ...
            using (ColorImageFrame colorFrame = e.OpenColorImageFrame())
            {
                if (colorFrame != null)
                {
                    ...
                    {
                        byte red = colorPixels[i];
                        byte green = colorPixels[i + 1];
                        byte blue = colorPixels[i + 2];

                        if (!ShouldMakeTransparent(tRed, tGreen, tBlue, red, green, blue, tolerance))
        ...
        private bool ShouldMakeTransparent(byte tRed,byte tGreen, byte tBlue ,byte red, byte green, byte blue, int tolerance)
        {
            return (Math.Abs(tRed - red) < tolerance &&
                    Math.Abs(tGreen - green) < tolerance &&
                    Math.Abs(tBlue - blue) < tolerance);
        }
